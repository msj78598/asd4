...
MODEL_PATH = "best.pt"  # تأكد من مسار النموذج المدرب
ML_MODEL_PATH = "isolation_forest_model.joblib"  # تأكد من مسار نموذج التعلم الآلي
    ...
    if st.sidebar.button("▶️ بدء تحليل البيانات", key="analyze_btn", help="انقر لبدء عملية تحليل البيانات"):
        with st.spinner('جاري تحليل البيانات...'):
            ...
            for idx, row in st.session_state.df.iterrows():
                ...
                meter_id = row['الاسم']  # عمود الاسم من البيانات
                lat, lon = row['y'], row['x']  # الإحداثيات
